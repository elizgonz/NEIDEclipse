# JULIA_NUM_THREADS=1 julia --startup-file=no plot_eclipse.jl
using CSV
using CUDA
using Revise, GRASS
using SPICE
...
time_stamps = (time_stamps_utc[1:length(time_stamps_utc)-28])

solar_horizon_angle = [13.69214256, 13.83337338, 13.97779268, 14.12369855, 14.26931778, 14.41821379, 14.56863172, 14.71874374, 14.87222317, 15.02726097, 15.18197332, 15.34014609, 15.49991472, 15.65933803, 15.82231716, 15.98693064, 16.15117858, 16.31908009, 16.48865543, 16.65991802, 16.83078732, 17.00544436, 17.18182936, 17.35779963, 17.53766089, 17.71929168, 17.90048562, 18.0856761,  18.27267846, 18.45922109, 18.64986805, 18.84236995, 19.03438833, 19.23062102, 19.42875231, 19.62637529, 19.82832467, 20.03221677, 20.23557463, 20.44337295, 20.65315849, 20.8649462, 21.07616302, 21.29197539, 21.50983468, 21.72709386, 21.94906676, 22.17313129, 22.39656481, 22.62483167, 22.85523465, 23.08497384, 23.31966724, 23.5565408, 23.79271569, 24.03396663, 24.27744111, 24.5201797,  24.7681169,  25.01832006, 25.26774752, 25.52249654, 25.77955268, 26.0357905,  26.29747282, 26.56150185, 26.82789041, 27.09339885, 27.36451523, 27.63802805, 27.91061039, 28.18892203, 28.46966454, 28.74942238, 29.03502939, 29.32309881, 29.61012523, 29.90311863, 30.19860264, 30.4929808,  30.79344113, 31.09641642, 31.39821816, 31.70621399, 32.01674479, 32.32602911, 32.64161548, 32.95975195, 33.28044239, 33.59978027, 33.92555687, 34.25389518, 34.58079442, 34.91422823, 35.25022529, 35.58469034, 35.92577932, 36.26942629, 36.61144158, 36.96016284, 37.31142946, 37.66095766, 38.0172657,  38.37609853, 38.73307886, 39.09690389, 39.46322465, 39.82757125, 40.19881757, 40.57252151, 40.94866823, 41.32266629, 41.70362072, 42.08696704, 42.46802471, 42.85606762, 43.24644145, 43.6343787,  44.02931761, 44.42651603, 44.82112206, 45.22273306, 45.62652147, 46.02755394, 46.43558098, 46.8456925, 47.25287707, 47.66703162, 48.0831668,  48.49619692]
...
Nsubgrid = 2
disk = GRASS.Eclipse.DiskParamsEclipse(N=N, Nt=Nt, Nsubgrid=Nsubgrid)
...
gpu_allocs = GRASS.Eclipse.GPUAllocsEclipse(spec, disk, length(spec.lines))
...
for t in 130:130 #48 & 96

    GRASS.Eclipse.calc_eclipse_quantities_gpu!(time_stamps[t], obs_long, obs_lat, obs_alt, lines, "SSD_4parameter", 0.0, ext_coeff_array[8], disk, gpu_allocs, 0.0)
    CUDA.synchronize()
    # parse out data
    ...
    xs, ys, zs = GRASS.Eclipse.calc_grid_edge_xyz(utc2et(time_stamps[t]), obs_long, obs_lat, obs_alt, disk, gpu_allocs)
    ...
    # get alt and az 
    out = get_alt_az.(utc2et(time_stamps[t]), ra, dec, obs_lat, obs_long)
    alt = getindex.(out, 1)
    az = getindex.(out, 2)
    ...
    ax1.set_xlabel("Azimuth", fontsize = 24)
    ax1.set_ylabel("Altitude", fontsize = 24)
    ...
    cbar.set_label(label="Velocity (m/s)", size = 24)
    ...
    for i in 1:size(dat, 1)
        for j in 1:size(dat, 2)
            ...
            ax1.fill(aa, bb, c=smap.to_rgba(dat[i,j]))
        end
    end
    ...
    az_min, az_max = minimum(filter(!isnan,az)), maximum(filter(!isnan,az))
    alt_min, alt_max = minimum(filter(!isnan,alt)), maximum(filter(!isnan,alt))

    ax1.set_xlim(az_min, az_max)
    ax1.set_ylim(alt_min, alt_max)
    az_mid = 0.5 * (az_min + az_max)
    Δx = az_max - az_min

    xticks = [az_mid - Δx/4, az_mid + Δx/4]
    ax1.set_xticks(round.(xticks; digits=1))
    alt_mid = 0.5 * (alt_min + alt_max)
    Δy = alt_max - alt_min

    yticks = [alt_mid - Δy/4, alt_mid, alt_mid + Δy/4]
    ax1.set_yticks(round.(yticks; digits=1))
    fig.savefig(joinpath(outdir, "eclipse_" * string(t) * ".png"), dpi = 600, bbox_inches="tight")
    plt.close()
end